...
def scrape_articles():
    ...
    # Step 2: Format the articles into a response-friendly structure
    response_data = [{'title': article[0], 'link': article[1]} for article in articles]

    # Step 3: Remove duplicates
    unique_articles = remove_duplicates(response_data)

    # Step 4: Extract text from unique articles
    text_data = extract_text_from_articles(unique_articles)

    # Step 5: Limit or pad the response to exactly 5 articles
    while len(text_data) < 5:
        # Add placeholder articles if fewer than 5 articles are available
        text_data.append({'title': 'Placeholder Title', 'link': 'https://example.com/placeholder'})

    # Trim the list to exactly 5 articles
